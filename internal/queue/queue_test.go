...
func TestQueueMaxRetries(t *testing.T) {
	...
	wq := New(workqueue.NewTypedMaxOfRateLimiter(
...
func TestQueueCustomRetries(t *testing.T) {
	...
	wq := New(&workqueue.TypedBucketRateLimiter[string]{Limiter: rate.NewLimiter(rate.Limit(1000), 1000)}, t.Name(), handler, shouldRetryFunc)
...
func TestForget(t *testing.T) {
	...
	wq := New(workqueue.DefaultTypedItemBasedRateLimiter[string](), t.Name(), handler, nil)
...
func TestQueueEmpty(t *testing.T) {
	...
	q := New(workqueue.DefaultTypedItemBasedRateLimiter[string](), t.Name(), func(ctx context.Context, key string) error {
		return nil
	}, nil)
...
type rateLimitWrapper struct {
	...
	rl           workqueue.TypedRateLimiter[string]
}

func (r *rateLimitWrapper) When(item string) time.Duration {
...
func (r *rateLimitWrapper) Forget(item string) {
	r.forgottenMap.Store(item, struct{}{})
	r.rl.Forget(item)
}

func (r *rateLimitWrapper) NumRequeues(item string) int {
...
func TestRateLimiter(t *testing.T) {
	...
	ratelimiter := &rateLimitWrapper{
		rl: workqueue.NewTypedItemFastSlowRateLimiter[string](1*time.Millisecond, 100*time.Millisecond, 1),
	}
